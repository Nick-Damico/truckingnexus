...
module UsersHelper
  ...
  def profile_button(user: current_user, **)
    return display_avatar(user) if user.avatar.present?

    display_placeholder(user, **)
  end
  ...
  def display_avatar(user, **_options)
    link_to(user_path(user), class: 'w-8 mr-2 rounded-full border-2 border-white border') do
  ...
  def display_placeholder(user, **options)
